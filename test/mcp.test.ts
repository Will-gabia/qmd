...
describe.skipIf(!!process.env.CI)("MCP HTTP Transport", () => {
  ...
  const origConfigDir = process.env.QMDX_CONFIG_DIR;
  ...
  beforeAll(async () => {
    ...
    process.env.QMDX_CONFIG_DIR = httpTestConfigDir;
  ...
  afterAll(async () => {
    ...
    if (origConfigDir !== undefined) process.env.QMDX_CONFIG_DIR = origConfigDir;
    else delete process.env.QMDX_CONFIG_DIR;
  ...
  test("POST /mcp initialize returns 200 JSON (not SSE)", async () => {
    ...
    expect(json.result.serverInfo.name).toBe("qmdx");
  });
