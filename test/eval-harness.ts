...
function runSearch(query: string): SearchResult[] {
  try {
    const output = execSync(
      `bun src/cli/qmdx.ts search "${query.replace(/"/g, '\\"')}" --json -n 5 2>/dev/null`,
...
function runQuery(query: string): SearchResult[] {
  try {
    const output = execSync(
      `bun src/cli/qmdx.ts query "${query.replace(/"/g, '\\"')}" --json -n 5 2>/dev/null`,
...
try {
  const status = execSync("bun src/cli/qmdx.ts status --json 2>/dev/null", { encoding: "utf-8" });
