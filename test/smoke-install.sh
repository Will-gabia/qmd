...
# install/runtime scenarios under npm, npx, and Bun. Supports optional qmdx embed
...
#   test/smoke-install.sh --with-embed            # also run tiny qmdx embed smoke tests
#   test/smoke-install.sh --with-gpu              # also probe GPU in doctor/embed scenarios
#   QMDX_SMOKE_GPU_BACKEND=cuda|vulkan|auto        # backend for --with-gpu (default: auto)
...
#     QMDX_SMOKE_GPU_ARGS='--device nvidia.com/gpu=all' test/smoke-install.sh --with-gpu
set -euo pipefail
...
IMAGE=${QMDX_SMOKE_IMAGE:-qmdx-smoke}
SCENARIO=all
DO_BUILD=1
WITH_EMBED=0
WITH_GPU=0
GPU_BACKEND=${QMDX_SMOKE_GPU_BACKEND:-auto}
...
gpu_args() {
  ...
  if [[ -n "${QMDX_SMOKE_GPU_ARGS:-}" ]]; then
    # shellcheck disable=SC2206
    echo ${QMDX_SMOKE_GPU_ARGS}
    return 0
  fi
  case "$CTR" in
...
fixture_setup='rm -rf /tmp/qmdx-fixture /tmp/qmdx-cache /tmp/qmdx-config /tmp/qmdx-models; mkdir -p /tmp/qmdx-fixture; printf "# Smoke Doc\n\nGPU and CPU embedding smoke test.\n" > /tmp/qmdx-fixture/doc.md; export XDG_CACHE_HOME=/tmp/qmdx-cache QMDX_CONFIG_DIR=/tmp/qmdx-config'

gpu_env() {
  case "$GPU_BACKEND" in
    auto|"") echo "" ;;
    cuda|vulkan|metal) echo "QMDX_LLAMA_GPU=$GPU_BACKEND" ;;
    *) echo "Unsupported QMDX_SMOKE_GPU_BACKEND=$GPU_BACKEND" >&2; exit 1 ;;
  esac
}

run_doctor_smoke() {
  local label="$1" bin="$2" extra_env="${3:-}"
  smoke_test_output "$label doctor" "QMDx Doctor" \
...
run_collection_smoke() {
  ...
  smoke_test "$label collection add/list/status" \
    "$fixture_setup; cd /tmp/qmdx-fixture; $extra_env $bin collection add . --name smoke; $extra_env $bin collection list; $extra_env $bin status"
}

run_embed_smoke() {
  local label="$1" bin="$2" extra_env="${3:-}"
  [[ $WITH_EMBED -eq 1 ]] || return 0
  smoke_test "$label qmdx embed tiny fixture" \
    "$fixture_setup; cd /tmp/qmdx-fixture; $extra_env $bin collection add . --name smoke; $extra_env $bin embed --max-docs-per-batch 1 --max-batch-mb 1; $extra_env $bin doctor"
}

run_runtime_matrix() {
  local label="$1" bin="$2" path_env="$3"
  smoke_test_output "$label qmdx help" "Usage:" "$path_env; $bin"
  run_doctor_smoke "$label auto" "$path_env; $bin"
  run_doctor_smoke "$label force-cpu" "$path_env; $bin" "QMDX_FORCE_CPU=1"
  run_collection_smoke "$label" "$path_env; $bin" "QMDX_FORCE_CPU=1"
  run_embed_smoke "$label force-cpu" "$path_env; $bin" "QMDX_FORCE_CPU=1"
...
run_node_scenario() {
  ...
  smoke_test "node sqlite-vec loads" \
    "export PATH=$NODE_BIN:\$PATH; NPM_GLOBAL=\$(npm root -g); node -e \"
      const {openDatabase, loadSqliteVec} = await import('\\$NPM_GLOBAL/qmdx/dist/db.js');
  ...
  smoke_test "node vitest store subset" \
    "export PATH=$NODE_BIN:\$PATH; cd /opt/qmdx && npx vitest run --reporter=verbose test/store.test.ts 2>&1 | tail -5"
}

run_npx_scenario() {
  local NODE_BIN='$(mise where node@latest)/bin'
  local bin='npm exec --yes --package /tmp/qmdx.tgz -- qmdx'
...
run_bun_scenario() {
  ...
  smoke_test "bun sqlite-vec loads" \
    "export PATH=$BUN_BIN:\$PATH; bun -e \"
      const {openDatabase, loadSqliteVec} = await import('\\$HOME/.bun/install/global/node_modules/qmdx/dist/db.js');
      const db = openDatabase(':memory:');
      loadSqliteVec(db);
      const r = db.prepare('SELECT vec_version() as v').get();
      console.log('sqlite-vec', r.v);
  ...
  smoke_test "bun test store subset" \
    "export PATH=$BUN_BIN:\$PATH; cd /opt/qmdx && bun test --preload ./src/test-preload.ts --timeout 30000 test/store.test.ts 2>&1 | tail -10"
}
